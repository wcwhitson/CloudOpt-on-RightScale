...
# 
################################################################################
log "Install Extension ClickTracking: Starting"
...
git install_dir do
  repository "https://gerrit.wikimedia.org/r/gitweb?p=mediawiki/extensions/ClickTracking.git"
  action :checkout
end

log "Install Extension ClickTracking: Ending"
...
template "#{node[:mediawiki][:installation_directory]}/#{node[:mediawiki][:ext_config_dir]}/ClickTrackingSettings.php" do
  source "ClickTracking.php.erb"
  mode "0644"
  owner "root"
  group "root"
end
